...
export const getPic = createAsyncThunk(
  "ownlink/getPic",
  async (email) => {
    const {data: { profilePic }} = await axios.get(process.env.REACT_APP_API + `/profilepic/${email}`);
...
export const getPicFromUsername = createAsyncThunk(
  ...
  async (username) => {
    const {data: { pic }} = await axios.get(process.env.REACT_APP_API + `/profilepicfromusername/${username}`);
    return pic;
  }
);
