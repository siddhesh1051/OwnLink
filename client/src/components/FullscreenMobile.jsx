...
const FullscreenMobile = () => {
  ...
  const handleGetProfilePicfromusername = async (username) => {
    ...
    const { data } = await axios.get(
      process.env.REACT_APP_API + `/profilepicfromusername/${username}`
    );
    // console.log(data)
  ...
  const handleGetBgfromusername = async (username) => {
    // console.log(email)
    const { data } = await axios.get(
      process.env.REACT_APP_API + `/bgfromusername/${username}`
    );
    // console.log(data)
    setBg(data.bg);
  };
  ...
  const fetchOwnlinkViews = async () => {
    try {
      const response = await axios.get(
        process.env.REACT_APP_API + `/increaseOwnlinkViews/${username}`
      );
  ...
  return (
    <div className="flex-1 bg-black">
      {emailFromUsername?.length === 0 || emailFromUsername === undefined ? (
        ...
        >
          <h1 class=" text-9xl font-extrabold text-indigo-300 tracking-widest">
            404
          </h1>
          ...
          <button class="mt-5">
            <a class="relative inline-block text-sm font-medium text-[#9358f9] group active:text-[#9358f9] focus:outline-none focus:ring">
              <span class="absolute inset-0 transition-transform translate-x-0.5 translate-y-0.5 bg-[#9358f9] group-hover:translate-y-0 group-hover:translate-x-0"></span>
        ...
        <div
          className={`screen-bg flex justify-start items-center w-screen h-screen flex-col gap-2 overflow-scroll no-scrollbar lg:rounded-[40px]`}
