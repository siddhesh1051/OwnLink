...
const LinearBar = ({ clicks, title, type, maxClicks }) => {
  ...
  return (
    <div className="flex flex-col mt-1 py-1 gap-2 px-4 ">
      <div className="flex items-center justify-between ">
        <div className="flex items-center gap-1">
          {type === "social" ? (
            <img src={getIcon(title)} alt="" className="inline scale-75 mr-2" />
          ) : (
            <LuLink className="inline mr-2" />
          )}
          <p
        ...
        <p className="text-md font-medium bg-clip-text text-transparent bg-gradient-to-r from-purple-100 to-blue-300">
          {clicks}
        </p>
      </div>
      <LinearProgress
        determinate
        value={getScaledValue(scaledValue)}
        thickness={10}
        sx={{ background: "#161C23" }}
      />
    </div>
  );
};
