...
const Orders = () => {
    ...
    const cancelOrder = async (order_id) => {
        try {
            const { data } = await axios.delete(`${process.env.REACT_APP_API}/cancelOrder/${order_id}`);
    ...
    useEffect(() => {
        const fetchMyOrders = async () => {
            try {
                const { data } = await axios.get(`${process.env.REACT_APP_API}/getOrders/${email}`);
    ...
    return (
        <div className='min-h-screen w-full bg-[#161a23] text-white flex flex-col gap-2 pt-8'>
            {myOrders.length !== 0 && <h1 className='text-3xl font-[poppins] font-bold text-center mb-4'>My Orders</h1>}
            <div>
