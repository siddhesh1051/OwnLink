  ...
  return (
    <div className="p-5 rounded-xl bg-[#222430] w-full  flex flex-col pb-8 ">
      ...
      <motion.div
        ...
        whileInView={{ y: 0, opacity: 1 }}
        viewport={{ once: true }}
        className="flex w-full px-6 py-2 my-2 justify-center items-center "
      >
      ...
      >
        {linksViews && linksViews.length !== 0 && (
          <h1 className="text-xl font-semibold bg-clip-text text-transparent bg-gradient-to-r from-purple-100 to-blue-300 mb-2 mt-4">
            Links
          </h1>
        )}
        ...
        {linksViews &&
          ...
          linksViews.map((linksView) => (
            <LinearBar
              clicks={linksView.linkClicks}
              title={linksView.title}
              key={linksView.index}
              maxClicks={maxLinksClicks}
              type="link"
            />
          ))}
      </motion.div>
